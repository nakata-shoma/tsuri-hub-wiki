---
status: 完了
category: glossary
last_updated: 2026-08-20
---

# 仕掛け

## 意味
仕掛けとは、魚を釣るために針・オモリ・サルカン・ウキなどのパーツを組み合わせたもので、道糸から先の部分全体を指す。

## 補足
- 使われる場面・具体例:
  - 釣り方によって呼び方が変わる(例: 投げ釣り仕掛け、ウキ釣り仕掛け)。
  - 狙う魚や釣り方に応じて、針のサイズ・オモリの重さ・ハリスの太さなどを組み合わせて作る。

## 関連用語
- [glossary_leader.md](glossary_leader.md)(リーダー)
- [glossary_tana.md](glossary_tana.md)(タナ)

## 出典
- [しかけ 仕掛け｜釣り用語辞典｜釣割](https://www.chowari.jp/sp/dictionary/detail.php?c=229)(確認日: 2026-08-20)
- [釣りの仕掛けとは？仕掛けを構成する釣具について解説｜SHIMANO シマノ](https://fish.shimano.com/ja-JP/content/beginners/fishingtackle/shikake/index.html)(確認日: 2026-08-20)
